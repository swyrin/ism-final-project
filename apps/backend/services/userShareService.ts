...
import * as userShareRepository from "@srv/repository/userShareRepository";
import { BadRequestError, NotFoundError } from "@srv/utils/HttpError";
import { v4 as uuidv4 } from "uuid";

export const createUserShareFile = async (user_id: string, file_id: string): Promise<string> => {
  ...
  try {
    ...
    await userShareRepository.createShareFile(user_id, share_id, {
      title: file.title,
      author: file.author,
    ...
    await userShareRepository.deleteShareFile(user_id, share_id);
    throw new BadRequestError("failed to share file.");
  }
};

export const importShareFile = async (
  user_id: string,
  document_id: string,
  category_id: string,
  share_id: string,
  ...
  const share = await userShareRepository.getShareFile(share_id);
  if (!share) {
